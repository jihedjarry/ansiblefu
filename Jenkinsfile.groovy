...
node {
	...
	/* Récupération de la version du pom.xml après modification */
    	def version = sh returnStdout: true, script: "/spring-blog-backend/pom.xml | grep -A1 '<artifactId>spring-blog-backend' | tail -1 |perl -nle 'm{.*<version>(.*)</version>.*};print \$1' | tr -d '\n'"
